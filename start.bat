...
REM ============================================================
REM  بيشغل السيرفرين مع بعض في نافذة واحدة
REM  بيقرا البورتات من ملف .env في جذر المشروع
REM ============================================================
...
REM ── قيم افتراضية لو .env مش موجود أو ناقص
...
REM ── شغّل API server في نافذة منفصلة
start "Forge API (port %FORGE_API_PORT%)" cmd /k ^
  "set OPENAI_API_KEY=%OPENAI_API_KEY%&& set FORGE_WORKSPACE_API_PORT=%FORGE_API_PORT%&& node -e \"const { createWorkspaceApiServer } = require('./code/src/workspace/apiServer'); createWorkspaceApiServer({ port: %FORGE_API_PORT% }).start()\""

REM ── انتظر ثانيتين عشان الـ API يبدأ
timeout /t 2 /nobreak >nul

REM ── شغّل Web server في نافذة منفصلة
start "Forge Web (port %FORGE_WEB_PORT%)" cmd /k ^
  "set OPENAI_API_KEY=%OPENAI_API_KEY%&& set FORGE_WEB_PORT=%FORGE_WEB_PORT%&& set FORGE_API_PORT=%FORGE_API_PORT%&& node web/server.js"
...
echo.
echo Both servers started. Closing this window won't stop them.
