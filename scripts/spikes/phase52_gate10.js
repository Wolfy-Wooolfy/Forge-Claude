...
// PHASE-52 — Gate #10: REAL auto web-discovery in the documentation citation pass.
//
// Proves the L-1 auto-discovery fix end-to-end on the REAL path:
//   fresh project, EMPTY KB (NO pre-seeded source) → seed loop → DOCUMENTATION → POST
//   /api/ai-os/project/document-project (real openai/gpt-4o doc-gen). The citation pass then,
//   PER §7.1 claim: zero_chunks → REAL research.search_web(claim) [Tavily] → REAL
//   kb.ingest_url(top result) into the project KB → re-retrieve → kb.cite → citations.jsonl →
//   §8 audit → advance DOCUMENTATION → QUALITY_JUDGE.
//
// Honest, fail-closed: report PASS/NOT as observed. Never rig a source or fake a PASS.
// Discovery provider = Tavily (TAVILY_API_KEY set, BRAVE_SEARCH_API_KEY UNSET). Spend cap
// $0.15 (STOP if exceeded); hard kill $3.00.
//
// DRY: PHASE52_GATE10_DRY=1 → $0 plumbing check (mock claim-free doc, no discovery, no real call).
//
// Evidence → artifacts/spikes/phase52_gate10/gate10_owner.json (+ step artifacts + citations.jsonl).
...
const KILL_BAR   = 3.00;
// PHASE-51 manual-source baseline (Azure REST Guidelines, general): the SECONDARY comparison bar.
const PHASE51_BASELINE = [0.475, 0.478];
...
// ── Fixtures (minimal health-check API — keeps the generated doc's §7.1 claim count small) ──
...
async function seedLoopToDocumentation(reg) {
  ...
  fs.writeFileSync(path.join(PROJECT_DIR, "project_state.json"), JSON.stringify({
    project_id: PROJECT_ID, project_name: "PHASE-52 Gate #10",
  ...
  fs.writeFileSync(MANIFEST_PATH, JSON.stringify({ built_at: new Date().toISOString(), files: manifestFiles.map(p => ({ path: p, sha256: "g52-fixture", line_count: 1 })) }, null, 2), "utf8");
  return manifestFiles;
}

async function main() {
  const reg = getDefaultRegistry();
  const DRY = !!process.env.PHASE52_GATE10_DRY;
  console.log("\n══ PHASE-52 — Gate #10 (" + (DRY ? "$0 DRY plumbing" : "REAL " + PROVIDER + "/" + MODEL + " + Tavily discovery") + ") ══\n");
  ...
  // Confirm the KB is genuinely empty.
  ...
  // Boot in-process apiServer (port 0; prod untouched).
  ...
  try {
    const docBody = DRY
      ...
      : { project_id: PROJECT_ID, loop_id: LOOP_ID };   // CLEAN body → real gpt-4o + real discovery
    console.log("\nStep 3 — POST /api/ai-os/project/document-project (" + (DRY ? "$0 mock" : "real " + MODEL + " + Tavily discovery") + ")");
    const t = Date.now();
    ...
    // Cap interaction: > CAP claims → surplus can't be searched → §8 HALT. STOP per §4 (no cap change).
    ...
    // provider(s) actually used by discovery searches — from the KB cost ledger web_search rows.
    ...
    // Copy citations.jsonl standalone (durable evidence).
    try {
      const srcCit = path.join(PROJECTS_ROOT, PROJECT_ID, "kb", "exports", "citations.jsonl");
      if (fs.existsSync(srcCit)) { fs.mkdirSync(EVIDENCE, { recursive: true }); fs.copyFileSync(srcCit, path.join(EVIDENCE, "citations.jsonl")); }
    } catch (_) {}

    // Spend delta.
    const spendAfter = readSpend();
    const costDelta  = Math.round((spendAfter.total_usd - spendBefore.total_usd) * 100000) / 100000;
    writeEvidence("step3_spend_after.json", { before: spendBefore, after: spendAfter, delta_usd: costDelta });
    ...
    const gateResult = {
      ...
      phase51_baseline: PHASE51_BASELINE,
      secondary_exceeds_baseline: exceedsBaseline,
      assertions: { audit_pass, advanced_true, advanced_to_qj, graph_qj, discovery_fired, discovery_cited, citations_written },
    ...
    console.log("  claims detected (N):", N, " | §8 status:", ca.status, " | advanced:", r.advanced, "→", r.advanced_to, " | graph:", graphAfter.current_state);
    console.log("  citation_pass:", JSON.stringify(cp));
