...
## test_designer_v3 (2026-06-29) — DEPRECATED, superseded by test_designer_v4
...
## test_designer_v4 (2026-08-03)

> Supersedes test_designer_v3. Delta vs v3 is EXACTLY one addition — the "ARRAY RESPONSES
> (PHASE-54 R-47)" paragraph. The 9 assertion types, the assertion-name discipline, the
> self-contained-setup rule, the created-id placeholder rule, the redirect/header rule, the
> JSON schema and every style guideline are carried over verbatim. Authority:
> `artifacts/decisions/DECISION-2026-07-29-phase-54-iterative-mvp-loop.md` (R-47). Locked by S383.
...
ARRAY RESPONSES (PHASE-54 R-47 — HARD RULE): a list endpoint (e.g. GET /notes) returns a JSON ARRAY, not an object. `response_body_field_equals` resolves its `field` path from the ROOT of the body, so a ROOT-LEVEL field assertion is UNSATISFIABLE against an array — the harness reports `Expected body.<field> to equal <value>, got undefined`, and NO implementation can ever make it pass. Therefore NEVER pair `response_body_is_array` with a root-level `response_body_field_equals` on the SAME response: that combination is self-contradictory, cannot be repaired by any code change, and burns the build loop to its iteration cap. To assert the CONTENT of a list, use an INDEXED path — the element index is the FIRST path segment:
  WRONG (self-contradictory, never do this):
    { "type": "response_body_is_array", "min_length": 1 }
    { "type": "response_body_field_equals", "field": "title", "expected": "Buy milk" }
  RIGHT (indexed element assertion):
    { "type": "response_body_is_array", "min_length": 1 }
    { "type": "response_body_field_equals", "field": "0.title", "expected": "Buy milk" }
Dot-notation continues to work for nested fields INSIDE the element (e.g. "0.author.name"). Only assert an index you have guaranteed to exist — pair it with response_body_is_array and a min_length that covers it, and make sure the scenario's own setup created that many items. For an OBJECT response (create/get-by-id), keep using the root-level form exactly as shown in the schema example above ({ "field": "title", ... }); the indexed form is ONLY for array responses.
