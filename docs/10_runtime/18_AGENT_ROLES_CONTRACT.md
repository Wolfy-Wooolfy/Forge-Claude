...
| §ARC-8 | `code/src/workspace/apiServer.js` — `POST /api/intake/upload` handler only | Direct `fs.writeFileSync(savedPath, fileBuffer)` for binary ZIP upload. The L2 `fs.write_file` tool converts content via `String(content)` before writing, which corrupts raw binary (ZIP) payloads. Scope: single `fs.writeFileSync` call in the upload handler; path constrained to `artifacts/uploads/` under root; filename sanitized. NOT a license for any other direct `fs.*` call in apiServer.js. | `DECISION-20260526-arc8-binary-upload-exemption.md` |
