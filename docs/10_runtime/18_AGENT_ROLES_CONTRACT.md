...
**OUT-OF-TRACK-A-SCOPE — Forge-v1 self-build CLI cluster.** The pre-L1–L4 self-build engine — `code/src/modules/*` (the self-build engine modules; **EXCEPT** the two LIVE governance modules `visionComplianceGate.js` + `specCompletenessEnforcer.js`, which ARE on the live surface and are Track-A-compliant after PHASE-37 STEP A), `code/src/execution/`, `code/src/cognitive/`, `code/src/orchestrator/`, `code/src/forge/` — ≈36 files / ≈198 direct `fs.*Sync` writes (+2 `child_process`, +1 `fetch`) — is **not governed by Track A**. It predates the Runtime Layers, is **unreachable from the live API** (verified by the full-live-surface import graph — see audit §3.5), and is **CLI-invokable only** (`bin/forge-run.js`, `bin/forge-autonomous-run.js`, `bin/forge-autonomy-step.js`). Direct `fs` / `child_process` / `fetch` is **tolerated there pending a future owner-gated migrate-or-retire decision**. This note is **NOT** a license for any direct side effect on the live runtime surface. Authorization: `DECISION-2026-06-18-phase-37-arc-drift-audit.md`.
