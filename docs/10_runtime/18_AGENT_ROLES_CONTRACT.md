...
| §ARC-3 | `code/src/runtime/builtproject/harness_runner.js` — `child_process.spawn` (server lifecycle: start, stdout capture, port polling, teardown). **Extended (install-time infrastructure, same family):** `bin/forge-install.js` — entry point, no direct fs/child_process; `scripts/install/preflight.js` — `child_process.execSync` (admin check, sc query, WMIC disk) + `net.createServer` (port check); `scripts/install/install_orchestrator.js` — `child_process.execSync` (robocopy, npm install, NSSM commands, service start) + `fs.*Sync` (dir creation, file checks); `scripts/install/rollback.js` — `child_process.execSync` (NSSM stop/remove, rmdir) + `fs.*Sync` (diagnostic dump write); `scripts/install/post_verify.js` — `child_process.execSync` (forge-doctor, sc query, sc queryex, taskkill) + `fs.*Sync` (evidence writes, uid_pin.json read, session file read) + `http` module (API calls to running server) | `DECISION-202605131800-phase-8-arc-3-spawn-exception.md` (original); `DECISION-2026-05-20T10-00-stage-12-7-amendment-automated-installer.md §6` (install-time extension — §ARC count stays 6) |
...
| §ARC-7 | `code/src/startup/env_loader.js` — `loadDotEnv(dir)` function only | Direct `fs.readFileSync` to read `.env` before the L2 Tool Runtime is loaded. `start-api.js` calls `loadDotEnv` at lines 16–17, before `apiServer.js` is required. At that point the Tool Registry does not exist — routing through `tools.fs.read_file` would cause a circular-dependency crash. Pre-runtime bootstrap pattern; same family as §ARC-6 / §X.3 (`metrics_initializer.js`). Scope: exactly one `fs.readFileSync` call inside `loadDotEnv`. NOT a license for any other `fs.*` call in this file or any other startup file. | `DECISION-2026-05-24T10-00-arc-7-env-loader-fs-exception.md` |
| §ARC-8 | `code/src/workspace/apiServer.js` — `POST /api/intake/upload` handler only | Direct `fs.writeFileSync(savedPath, fileBuffer)` for binary ZIP upload. The L2 `fs.write_file` tool converts content via `String(content)` before writing, which corrupts raw binary (ZIP) payloads. Scope: the `fs.writeFileSync(savedPath, fileBuffer)` call **plus its companion `fs.mkdirSync(uploadsDir, { recursive: true })`** — both within the upload handler only (the mkdir prepares the same `artifacts/uploads/` target for the same binary-ZIP write); path constrained to `artifacts/uploads/` under root; filename sanitized. NOT a license for any other direct `fs.*` call in apiServer.js. | `DECISION-20260526-arc8-binary-upload-exemption.md`; scope-widening (companion mkdir) `DECISION-2026-06-18-phase-37-arc-drift-audit.md` |
| §ARC-9 | `code/src/runtime/permission/permissionPolicy.js` (L3 decision self-audit), `code/src/runtime/audit/toolAuditLog.js` (cross-cutting tool-audit JSONL), `code/src/providers/_contract/providerTrace.js` (L1 forensic trace + cost ledger), `code/src/runtime/logging/metrics_initializer.js` (pre-runtime boot hook), `code/src/runtime/doctor/runDoctor.js` (L4 diagnostics report + status patch) | Direct `fs.*Sync` (mkdir / append / write) for runtime self-instrumentation at or below the L2 seam. Routing through the L2 `fs.write_file` tool is **circular**: every L2 invocation passes through L3 `permissionPolicy.authorize()` (→ `toolAuditLog` + permission audit), so audit writes cannot themselves be L2 calls; `providerTrace` runs inside the L1 adapter beneath L2; `metrics_initializer` is a boot hook that runs before the Tool Registry exists; `runDoctor` is the L4 bootstrap/diagnostics report. Re-entrancy prevention + pre-runtime bootstrap — same family as §ARC-1 / §ARC-6 / §ARC-7. 12 write-occurrences across 5 files. NOT a license for `fs.*` direct use outside §ARC-9 scope. | `DECISION-2026-06-18-phase-37-arc-drift-audit.md` |
| §ARC-10 | `code/src/runtime/builtproject/verdict_aggregator.js`, `code/src/runtime/builtproject/loopback_signal.js` | Direct `fs.mkdirSync` / `fs.writeFileSync` into the **built project's** `forge_tests/` directory (an EXTERNAL project root, NOT the Forge workspace) — writing the harness `last_report.json` + `loopback_signal.json`. The L2 `fs.write_file` tool / L3 policy are scoped to the Forge root and would deny writes to an arbitrary external project root. Built-project harness infrastructure — same family as §ARC-2 (test infra) / §ARC-3 (harness). 6 writes / 2 files. (Supersedes the previous mis-cited inline "§ARC-1" comments in both files, corrected to §ARC-10 by this audit.) NOT a license for `fs.*` direct use outside §ARC-10 scope. | `DECISION-2026-06-18-phase-37-arc-drift-audit.md` |

### §ARC Track A Scope — Live Runtime vs. Legacy Self-Build Domain

> Scope clarification (NOT a numbered §ARC entry). §ARC count = **10** (§ARC-1 .. §ARC-10).

Track A (no direct `fs.*Sync` / `child_process` / `fetch()` / `new OpenAI()` outside sanctioned homes) governs the **live runtime surface**: `code/src/workspace/apiServer.js` (all routes) + `code/src/ai_os/**` + `code/src/runtime/**`. On that surface the only sanctioned direct-side-effect sites are the L2 tool homes (`code/src/runtime/tools/*`) and the numbered §ARC-1 .. §ARC-10 exceptions.

**OUT-OF-TRACK-A-SCOPE — Forge-v1 self-build CLI cluster (RETIRED in PHASE-38).** The pre-L1–L4 self-build engine — `code/src/modules/*` (the self-build engine modules; **EXCEPT** the two LIVE governance modules `visionComplianceGate.js` + `specCompletenessEnforcer.js`, which ARE on the live surface, are Track-A-compliant after PHASE-37 STEP A, and are **PRESERVED**), `code/src/execution/`, `code/src/cognitive/`, `code/src/orchestrator/`, `code/src/forge/` — ≈36 files / ≈198 direct `fs.*Sync` writes (+2 `child_process`, +1 `fetch`) — predated the Runtime Layers, was **unreachable from the live API** (verified by the full-live-surface import graph — see audit §3.5), and was CLI-invokable only. It was **RETIRED in PHASE-38** (see `DECISION-2026-06-19-phase-38-legacy-cluster-retire.md`): the cluster (44 source files + 4 dead CLI bins — `forge-run`, `forge-autonomous-run`, `forge-autonomy-step`, `forge-build-state`) was deleted from the active tree; git history preserves it. Only the two LIVE governance modules remain under `code/src/modules/`. This note is **NOT** a license for any direct side effect on the live runtime surface. Authorization: `DECISION-2026-06-18-phase-37-arc-drift-audit.md` (audit) + `DECISION-2026-06-19-phase-38-legacy-cluster-retire.md` (retirement).
