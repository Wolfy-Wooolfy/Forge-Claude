"use strict";

const fs   = require("fs");
const path = require("path");

const { defineAdapter } = require("./_adapter_contract");

const ADAPTERS_DIR = path.join(__dirname, "adapters");

let _cache    = null;
let _warnings = [];

// ── Registry load ─────────────────────────────────────────────────────────────

function _loadAdapters() {
  const map   = new Map();
  const warns = [];

  let files;
  try {
    files = fs.readdirSync(ADAPTERS_DIR).filter(f => f.endsWith("_adapter.js"));
  } catch (err) {
    return { map, warns: ["adapters dir not found: " + err.message] };
  }

  for (const file of files) {
    let adapter;
    try {
      adapter = require(path.join(ADAPTERS_DIR, file));
    } catch (err) {
      warns.push("failed to load adapter '" + file + "': " + err.message);
      continue;
    }

    if (!adapter || typeof adapter.id !== "string" || !adapter.id.trim()) {
      warns.push("adapter '" + file + "' missing id — skipped");
      continue;
    }
    if (typeof adapter.available !== "function") {
      warns.push("adapter '" + adapter.id + "' missing available() — skipped");
      continue;
    }
    if (typeof adapter.invoke !== "function") {
      warns.push("adapter '" + adapter.id + "' missing invoke() — skipped");
      continue;
    }

    if (map.has(adapter.id)) {
      warns.push("duplicate adapter id '" + adapter.id + "' in '" + file + "' — skipped");
      continue;
    }

    map.set(adapter.id, adapter);
  }

  return { map, warns };
}

// ── Public API ────────────────────────────────────────────────────────────────

function getAdapters() {
  if (!_cache) {
    const { map, warns } = _loadAdapters();
    _cache    = map;
    _warnings = warns;
  }
  return _cache;
}

function pickAdapter(providerId) {
  return getAdapters().get(providerId) || null;
}

function listAdapters() {
  return Array.from(getAdapters().keys());
}

function getWarnings() {
  getAdapters();
  return _warnings.slice();
}

function resetAdapterCache() {
  _cache    = null;
  _warnings = [];
}

module.exports = { getAdapters, pickAdapter, listAdapters, getWarnings, resetAdapterCache };
