...
const gem_adapter = {
  ...
  async install(packages, ctx) {
    ...
    const argv   = ["gem", "install"].concat(packages);
  ...
  async remove(packages, ctx) {
    ...
    const argv   = ["gem", "uninstall"].concat(packages);
    const result = await _spawnInWorkspace(project_id, argv, ctx);
  ...
  async audit(ctx) {
    ...
    const result = await _spawnInWorkspace(project_id, ["bundle", "audit"], ctx);
