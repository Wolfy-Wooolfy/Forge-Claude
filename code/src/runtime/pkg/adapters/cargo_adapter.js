...
const cargo_adapter = {
  ...
  async install(packages, ctx) {
    ...
    const argv   = ["cargo", "add"].concat(packages);
  ...
  async remove(packages, ctx) {
    ...
    const argv   = ["cargo", "remove"].concat(packages);
    const result = await _spawnInWorkspace(project_id, argv, ctx);
  ...
  async audit(ctx) {
    ...
    const result = await _spawnInWorkspace(project_id, ["cargo", "audit", "--json"], ctx);
