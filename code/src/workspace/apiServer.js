...
function createWorkspaceApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && pathname === "/api/ai-os/project/review-spec") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.reviewSpec(body));
        return;
      }

      if (req.method === "POST" && pathname === "/api/ai-os/project/estimate-cost") {
        const body = await readBody(req);
      ...
      if (req.method === "POST" && pathname === "/api/ai-os/project/design-tests") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.designTests(body));
        return;
      }

      if (req.method === "POST" && pathname === "/api/ai-os/project/build-project") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.buildProject(body));
        return;
      }
      ...
      if (req.method === "POST" && pathname === "/api/ai-os/project/review-project") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.reviewProject(body));
        return;
      }
      ...
      if (req.method === "POST" && pathname === "/api/ai-os/project/judge-quality") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.judgeQuality(body));
        return;
      }
      ...
      if (req.method === "POST" && pathname === "/api/ai-os/project/finalize-deliverable") {
        const body = await readBody(req);
        sendJson(res, 200, await conversationEngine.finalizeDeliverable(body));
        return;
      }
