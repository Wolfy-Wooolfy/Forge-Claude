...
//
// Verifies the test_designer role is wired to the strengthened test_designer_v4
// prompt (assertion-name discipline), proving the role → loader → v3 chain that
// W-1 changes — NOT only a raw read of the .md:
//   (a) the role's declared system_prompt_id === "test_designer_v4";
//   (b) the role source resolves its prompt via loadPrompt("test_designer_v4")
//       (the same resolution the role uses);
//   (c) loadPrompt("test_designer_v4") (that same resolution) returns text that
...
async function runS340TestDesignerPromptVersion() {
  ...
  // Requiring the role triggers loadPrompt("test_designer_v4") at module-load; if
  ...
  const active_prompt_id_is_v3 = role.system_prompt_id === "test_designer_v4";

  // (b) role source resolves its prompt via loadPrompt("test_designer_v4").
  ...
  try {
    ...
    role_loads_v3_via_loader = roleSrc.includes('loadPrompt("test_designer_v4")');
  } catch (_e) { /* false */ }

  // (c)+(d) resolve v3 the same way the role does and inspect its content.
  const v3 = loadPrompt("test_designer_v4");
