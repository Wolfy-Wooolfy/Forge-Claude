...
// ── stage_11_1_live_runner.js ─────────────────────────────────────────────────
// Live demo for PHASE-11 Stage 11.1 — Python Analyzer + Reverse Vision.
// Single role invocation (no orchestration loop). Simpler than live_ratification_runner.
//
// Flow:
//   1. project.intake_zip   — copy fixture_pycli → artifacts/projects/<id>/source/
//   2. project.analyze_source — SourceTreeAnalysis
//   3. role.invoke reverse_vision — real OpenAI call (gpt-4o), OUTPUT_SCHEMA validated by role
//   4. fs.write_file vision.md (unlocked) — INTAKE_CONTRACT §5 auto-lock PROHIBITED
//   5. agent.read_ledger — report final cost
//
// Kill switch: polls cost every 5s, aborts if >= KILL_THRESHOLD_USD ($1.50).
// Hard cap: $2.00 — checked after role.invoke completes.
//
// Track A: no direct fs.*, no new OpenAI(), no child_process, no fetch().
//   All state via registry tools. All LLM calls via role.invoke → agent.invoke → adapter.

const path = require("path");
const { getDefaultRegistry } = require("../../runtime/tools/_registry");

const PROJECT_ID         = "stage_11_1_live_demo";
const PROVIDER           = "openai";
const MODEL              = "gpt-4o";
const KILL_THRESHOLD_USD = 1.50;
...
function _log(msg) { console.log("[stage-11-1-live] " + msg); }
...
// ── Simple kill switch (no orchestration.abort — no loop_id in this flow) ────
...
async function runStage11_1LiveDemo() {
  const ROOT    = process.cwd();
  const ctx     = { root: ROOT, project_id: PROJECT_ID };
  const startTs = Date.now();

  _log("═══════════════════════════════════════════════════════════");
  _log("PHASE-11 Stage 11.1 Live Demo");
  ...
  // ── Step 1: intake_zip ──────────────────────────────────────────────────────
  _log("Step 1/5: intake_zip — copying fixture_pycli...");
  ...
  _log("Step 3/5: role.invoke reverse_vision (REAL LLM CALL) ...");
  ...
  // Extract InferredVision from role.invoke output (role_id is merged in, filter it out)
  const iv = {
...
module.exports = { runStage11_1LiveDemo, PROJECT_ID, KILL_THRESHOLD_USD, HARD_CAP_USD };
