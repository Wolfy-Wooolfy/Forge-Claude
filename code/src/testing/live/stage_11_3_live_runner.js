...
// ── stage_11_3_live_runner.js ─────────────────────────────────────────────────
// Live demo for PHASE-11 Stage 11.3 — Go Analyzer Extension.
// Single role invocation (no orchestration loop).
//
// Flow:
//   1. project.intake_zip   — copy fixture_gocli → artifacts/projects/<id>/source/
//   2. project.analyze_source — SourceTreeAnalysis (go language, go.mod parsed, AST symbols)
//   3. role.invoke reverse_vision — real OpenAI call (gpt-4o), v2 prompt
...
const PROJECT_ID         = "stage_11_3_live_demo";
...
function _log(msg) { console.log("[stage-11-3-live] " + msg); }
...
async function runStage11_3LiveDemo() {
  const ROOT    = process.cwd();
  const ctx     = { root: ROOT, project_id: PROJECT_ID };
  const startTs = Date.now();

  _log("═══════════════════════════════════════════════════════════");
  _log("PHASE-11 Stage 11.3 Live Demo");
  ...
  // ── Step 1: intake_zip ──────────────────────────────────────────────────────
  _log("Step 1/5: intake_zip — copying fixture_gocli...");
  ...
  if (sourceTree.detected_framework) {
    ...
    _log("  Framework detected: null (expected — no framework detection for Go)");
  }
  if (sourceTree.entry_points && sourceTree.entry_points.length > 0) {
    _log("  Entry points: " + sourceTree.entry_points.join(", "));
  }
  if (sourceTree.manifest_files && sourceTree.manifest_files.go_mod) {
    const gm = sourceTree.manifest_files.go_mod;
    _log("  go.mod: module=" + gm.module_path + ", go=" + gm.go_version);
  }

  // ── Step 3: role.invoke reverse_vision (real OpenAI call) ──────────────────
  _log("Step 3/5: role.invoke reverse_vision (REAL LLM CALL — v2 prompt)...");
  ...
  _log("  domain:             " + iv.domain + (iv.domain === "cli_tool" ? " ✓" : " ← CHECK"));
  ...
  _log("  domain:         " + iv.domain + (iv.domain === "cli_tool" ? " ✓ (expected)" : " ← SOFT REGRESSION — record in closure §5"));
  _log("═══════════════════════════════════════════════════════════");
...
module.exports = { runStage11_3LiveDemo, PROJECT_ID, KILL_THRESHOLD_USD, HARD_CAP_USD };
