...
(async () => {
  ...
  console.log("M1: 128 scenario JSON files present and parse");
  {
    ...
    check("M1 exactly 128 scenario files",
      files.length === 128,
      "found " + files.length);
    ...
    check("M1 all expected IDs present (S01–S128)", allIds,
  ...
  console.log("\nM4: runScenarios returns stable report schema");
  {
    ...
    check("M4 report has 128 scenarios",
      Array.isArray(report.scenarios) && report.scenarios.length === 128,
