...
- W-D — service_lifecycle pm2-aware + pm2-canonical boot. service_lifecycle.js gained a pm2 detection block via the existing shell.run_read_only seam (no new §ARC) -> PASS "running via pm2". pm2 established as the single boot path: pm2 start + pm2 save, and the \ForgeAPI logon task repurposed from raw `node start-api.js` to `node "<pm2 CLI>" resurrect` (retiring the competing direct-node :3100 binder). Boot-start + crash-restart proven by deterministic OS tests. (Amendment A-1.)
...
## 5. Outcome
Forge is production-hardened on local Windows: the OpenAI secret lives encrypted in the OS keychain (off-disk); a single pm2-canonical boot path provides boot-start (logon resurrect) + crash-restart (pm2 autorestart), both OS-tested; the stale D:\ForgeAI copy is gone; the SU suite is genuinely hermetic.
