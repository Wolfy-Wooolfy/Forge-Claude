...
A-6 addendum (2026-07-05, same day): the owner-run one-click gate immediately caught a PRE-EXISTING defect in RUN_FORGE.bat — `pm2 ping` (self-heal block) is invoked without `call`; since pm2 resolves to the npm pm2.cmd shim, the script chains into it and terminates when it returns, so execution dies before `pm2 start` is ever reached (the window shows only the orphan-cleanup line, then closes). Fix: add `call` to the ping invocation — a one-word control-flow fix, within A-6 §2 scope (no change to start mechanics). This finding retroactively re-attributes the W-D Step-1 observation ("RUN_FORGE.bat output got cut after the orphan-cleanup line") to this bug rather than PowerShell pipe truncation. Second instance this phase of the real owner path catching what synthetic runs missed (after A-5's console-close kill-vector) — reaffirming the owner-run gate as the true closure test.
