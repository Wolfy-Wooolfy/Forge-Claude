...
**Group 2 subtotal: 37 files / 200 writes.** Classification = **(A) LEDGER-GAP under a NEW "pre-L2 legacy domain" family**, *with the caveat that a strict 4-family reading would call them (B)*. They are NOT live-pipeline drift (you would not migrate dead/legacy code to `reg.invoke`); the correct resolution is an **owner/CTO decision** (ledger-as-legacy vs. quarantine vs. migrate). This is the audit's central open question (§7/§8).

### 4c. `apiServer.js` residual (live domain)

`apiServer.js` has 4 pattern-occurrences: `2200` (a §ARC-8 **comment** — not code), `2210` writeFileSync (§ARC-8 AUTHORIZED), `2206` mkdirSync (companion *inside* the §ARC-8 upload handler — within intent but not named by the ledger wording), and `93` `ensureDir` mkdir (used at `624` for `projectsRoot` scaffolding — minor server-infra dir creation). Judgment: `2206` → §ARC-8 wording extension; `93` → minor LEDGER-GAP / §ARC-8-extension (server dir scaffolding), **not** drift.

### 4d. TRUE-DRIFT (B)

**Zero.** No file in the live AI-OS pipeline (`ai_os/conversationEngine.js`, `runtime/orchestration/*`, `workspace/apiServer.js` business logic) performs ordinary direct-fs writes that should route through `reg.invoke`. The live runtime is Track A clean: `conversationEngine.js`'s `fs.*Sync=2` are **reads** (permitted), and orchestration writes all go through `reg.invoke`. This is a reassuring result.

---

## 5. TRUE-DRIFT Table

| File | Count | Why it violates | Migration note |
|---|---|---|---|
| *(none)* | 0 | — | The live pipeline routes all side effects through `reg.invoke`. |
...
1. **The §ARC count should increase from 8.** Propose **+2 firm new entries** plus **1 owner-decision entry**:
   ...
   - **DECISION-GATED §ARC-11 (or a "legacy domain" exclusion):** the pre-L2 self-build engine (`modules/`, `execution/`, `cognitive/`, `orchestrator/`, `forge/`) — 37 files / 200 writes + 2 `child_process` + 1 `fetch`. **Do not auto-ledger;** requires the §8 owner decision.
2. **Wording refinements** to existing entries: §ARC-8 (cover companion `mkdirSync@2206`; address `apiServer:93` `ensureDir`), §ARC-5 (note `execFile` lives in platform modules, `secret_provider` is dispatcher), §ARC-3 (note install scripts deprecated-but-present).
3. **Files needing TRUE-DRIFT migration: none.** The live pipeline is clean.
...
1. **Legacy self-build engine (200 writes / 37 files):** ledger it as a bounded "legacy domain" exception, **quarantine** it (mark out-of-Track-A-scope), or **migrate** it? Is it still invoked at all (e.g., `bin/forge` → `orchestrator/runner.js` / `autonomous_runner.js` self-build CLI), or is it dead?
